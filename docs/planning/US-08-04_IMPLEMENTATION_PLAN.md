...
|---|---|---|---|
| 0.3.0 | 2026-09-11 | Codex | Recorded exact `US0804-ART-01 candidate v1` approval and implementation start SHA `4b1dee1...`. Promoted backdrop and transparent 12-item atlas; added immutable equipped-room projection, isolated controller, Home focus lifecycle, deterministic layered rendering, explicit `room`/`focus` scene modes, accessibility summary, tests and smoke guide. Owner screenshot review refined mug/plant desk anchors and removed normal-state Pet/item copy for the simple virtual-pet-machine theme while preserving semantic accessibility. All item frames now scale from measured room dimensions. Room mode fits the full backdrop and scales Cat down; focus mode renders Cat only. Full quality 175 files / 907 tests and Android/iOS exports PASS; owner/manual smoke remains NOT_RUN. |
