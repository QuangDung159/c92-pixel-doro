...
const issueCopy: Record<SettingsIssueCode, string> = {
  ...
  SETTINGS_WRITE_FAILED:
    "Chưa lưu được thay đổi. Giá trị đã commit vẫn được giữ.",
  SETTINGS_REFRESH_FAILED:
    "Thay đổi có thể đã lưu nhưng màn hình chưa đồng bộ. Hãy thử lại.",
  ANALYTICS_CLEANUP_REQUIRED:
    "Analytics đã tắt. Cần thử lại để hoàn tất xóa hàng đợi và đổi mã ẩn danh.",
  NOTIFICATION_PERMISSION_FAILED:
    "Chưa đọc được quyền thông báo của hệ điều hành.",
  NOTIFICATION_OS_BLOCKED:
    "PixelDoro đang bật nhắc việc, nhưng hệ điều hành đang chặn thông báo.",
  NOTIFICATION_SYNC_FAILED:
    "Tùy chọn đã lưu nhưng lịch thông báo chưa đồng bộ.",
  SYSTEM_SETTINGS_UNAVAILABLE:
    "Không mở được Cài đặt hệ thống trên thiết bị này.",
  RESET_FAILED:
    "Chưa thể xóa dữ liệu. Không có trạng thái thành công giả; dữ liệu đã commit vẫn được bảo vệ.",
};
...
export const SettingsIssueBanner = ({
  ...
  <PixelPanel tone="danger">
    ...
    {issue !== "NOTIFICATION_OS_BLOCKED" &&
    issue !== "SYSTEM_SETTINGS_UNAVAILABLE" ? (
...
export const FocusDefaultsSection = ({
  ...
  <PixelPanel>
    ...
    <Text style={styles.help}>
      Áp dụng khi mở một Focus Setup mới; không đổi phiên đang chạy.
    </Text>
    <DurationControl
      max={120}
      min={15}
...
export const PreferenceSection = ({
  ...
  <PixelPanel>
    ...
    <ToggleRow
      body="Âm báo ngắn khi hoàn tất và nhận thưởng mới."
      label="Âm thanh"
      onValueChange={onSetSound}
      value={soundEnabled}
    />
    <View style={styles.divider} />
    <ToggleRow
      body="Rung nhẹ cho hành động quan trọng; nội dung không phụ thuộc vào rung."
      label="Rung phản hồi"
      onValueChange={onSetHaptics}
...
const permissionCopy = (
  permission: FocusNotificationPermission | "checking" | "unavailable",
) => {
  if (permission === "allowed") return "Hệ điều hành: đã cho phép.";
  if (permission === "denied")
    return "Hệ điều hành: đang chặn. Bạn có thể mở Cài đặt hệ thống.";
  if (permission === "undetermined")
    return "Hệ điều hành: chưa được hỏi; chỉ hỏi khi bạn chủ động bật.";
...
export const NotificationSection = ({
  ...
  <PixelPanel>
    ...
    <ToggleRow
      body="Nhắc khi Focus hoặc Break kết thúc."
      label="Nhắc kết thúc phiên"
...
export const DataControlSection = ({
  ...
  <PixelPanel tone="strong">
    ...
    <ToggleRow
      body="Dữ liệu sử dụng ẩn danh, không có tài khoản. Tắt sẽ dừng capture, xóa hàng đợi và đổi mã ẩn danh."
      label="Anonymous analytics"
      onValueChange={onSetAnalytics}
      value={analyticsEnabled}
    />
    <View style={styles.divider} />
    <Text style={styles.dangerTitle}>Vùng nguy hiểm</Text>
    <Text style={styles.help}>
      Xóa lịch sử, tiến trình, Coin, vật phẩm và tùy chọn local; catalog và
      schema hợp lệ được giữ lại.
    </Text>
    <SecondaryButton
      busy={resetBusy}
      label="Xóa toàn bộ dữ liệu local"
...
export const FeedbackEntrySection = ({
  ...
  <PixelPanel>
    ...
    <Text style={styles.help}>
      Chia sẻ điểm trải nghiệm và lời nhắn tùy chọn trực tiếp với team
      PixelDoro.
    </Text>
